...
//  (1)
        if(request.getType() == Request.requestsType.registration_salary_income){
            throw new CustomExeption("Utilizatorul de tip persoana nu poate inainta o cerere de tip inregistrare venit salarial");
        }
        if(request.getType() == Request.requestsType.replacement_of_the_student_card){
            throw new CustomExeption("Utilizatorul de tip persoana nu poate inainta o cerere de tip inlocuire carnet de elev");
        }
        if(request.getType() == Request.requestsType.creation_of_articles_of_incorporation){
            throw new CustomExeption("Utilizatorul de tip persoana nu poate inainta o cerere de tip creare act constitutiv");
        }
        if(request.getType() == Request.requestsType.renewal_of_the_authorization){
            throw new CustomExeption("Utilizatorul de tip persoana nu poate inainta o cerere de tip reinnoire autorizatie");
        }
        if(request.getType() == Request.requestsType.registration_of_pension_coupons){
            throw new CustomExeption("Utilizatorul de tip persoana nu poate inainta o cerere de tip inregistrare cupoane de pensie");
        }
//  (2)
...
//  (3)
        ...
        ManagementPrimarie.officePerson.requests_priority.add(request);
...
//  (2)
        for(Request request : ManagementPrimarie.officePerson.requests_priority)
            if(request.getDate().equals(date)) {
                ManagementPrimarie.officePerson.requests_priority.remove(request);
                break;
            }
    }
